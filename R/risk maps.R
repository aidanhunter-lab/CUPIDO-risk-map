...
# (1) Biota interacting with plastic directly.
# (2) Plastic interacting with existing stresses -- multi-stressor risk.
...
nranks <- 5
# Display NA value on the maps rather than leave them blank
showNA <- TRUE
na.colour <- 'grey40'
# For SST and pH, colour only the statistically significant trends
significantTrendsOnly <- TRUE
# Rounding was useful for unit of ship time but not for person time.
# Keep roundShipTime set as FALSE
roundShipTime <- FALSE
# Limit domain of interaction maps to the minimum latitudinal extent of all data
reduceDomain <- TRUE
# Tool-tip is only relevant to the interactive map. Keep set to TRUE.
loadTooltipFromFile <- TRUE
# As above, only relevant to interactive map.
displayAllLitterTypes <- TRUE
# Select resolution of maps: either '9x3' or '3x1'.
res <- '9x3'
...
nc <- nc_cells # map data

# Filter by month: select 1, 2, 3, and/or 'all'.
mon <- 'all'
...
# Filter by ship type. There are a few options ('all', 'fishing', 'tourism',
# 'supply', 'research', 'other'), but choosing all ship activity makes sense
# when the unit if activity is person days.
ship_type <- 'all'
ship <- subset(ship_poly, activity == ship_type)

if(reduceDomain){
  # Restrict latitudinal domain of risk maps to the latitudinal extent of the
  # combined shipping and facility data. Shipping data is only available south
  # of 60 degrees south, so this is the domain limit.
  lat_lim_ship <- range(st_coordinates(st_transform(
    ship, crs = crs_world))[,'Y'])
  lat_lim_station <- range(st_coordinates(st_transform(
    STATIONS_sf, crs = crs_world))[,'Y'])
  excludeAnyStations <- lat_lim_station[2] > lat_lim_ship[2] # 
  # excludeAnyStations # Good. We don't need to exclude facilities due to
                       # restricting latitudinal domain.
  excludeCells <- grid_cells_all$cell_index[
    !{grid_cells_all$cell_index %in% ship$cell_index}]
}
...
# Discard grid cells that are not observed in any data set (i.e., underneath
# the land)
...
# High productivity regions are associated with greater potential for
# environmental risk, so microplastic risk increases with chlorophyll
# concentration.
...
yr <- y # There are no obvious outliers
yl <- log10(yr)
...
# Every data point is assigned a score (continuous value) according to the
# ranking thresholds 
for(i in 1:nranks){
  if(i < nranks){
...
# Convert continuous scores to discrete ranks
...
plt_chl_raw <- 
  ggplot(chl) + 
  geom_point(aes(x = index, y = value, fill = rank, colour = rank), shape = 21,
             size = 3, alpha = 0.8) +
  ...
  annotate('text', x = 1, y = r[2:nranks], label = as.character(r[2:nranks]),
           vjust = 0, hjust = 0) +
...
# Krill is a Southern Ocean key-stone species. Microplastic poses greater
# environmental risk in regions with high krill density.
# Take a similar approach to chlorophyll but note that there are grid cells with
# no data and grid cells with measurement zeros. Grid cells without data are
# left blank (gray) while grid cells with measurement zeros are assigned the
# lowest rank.
...
# to rising temperature, but the SST data record regions of both rising and
# falling temperatures.
# Set cells with declining or non-significant sst trends to minimum risk.

# Index grid cells with trend of increasing temperature (all p-values)
sst_pos <- sst$value > 0
...
# Other cells need ranked based on size of positive trend
sst_selected_cells <- !min_risk
...
y_r <- yl[-ol]
yrange <- range(y_r)
# Set all positive trends in sst as rank 2-5, leaving rank 1 for the
...
r[2:nranks] <- round(r[2:nranks] / {orderOfMagnitude(r[2:nranks]) / 10}) *
  {orderOfMagnitude(r[2:nranks]) / 10}
...
# Map plot
plt_sst <- plot_fun(sst_na, Title = 'SST rising trends',
                    legend_title = 'stress')
plt_sst$plot
...
plt_sst_raw <- 
  ...
  guides(fill = guide_legend(reverse = TRUE),
         colour = guide_legend(reverse = TRUE)) +
  geom_hline(yintercept = r[2:nranks], linetype = 3) +
  annotate('text', x = 1, y = r[3:nranks], label = as.character(r[3:nranks]),
           vjust = 0, hjust = 0) +
  annotate('text', x = 1, y = r[2], label = as.character(r[2]), vjust = 1,
           hjust = 0) +
  xlab(expression(grid ~ cell ~ index)) +
...
# Environmental risk due to trends in pH is treated similarly to trends in SST.
# With pH, however, it is declines that usually perceived as environmentally
# problematic, so our treatment of this variable is inversed to our treatment
# of SST. (Of course, both increases and decreases are potentially problematic,
# or indeed beneficial, to different species, so assuming that movement of the
# variable in one particular direction is bad, while movement in the other
# direction is benign is rather silly! ;) )

# Index grid cells with trend of decreasing pH (all p-values)
pH_neg <- pH$value < 0
...
x_r <- 1:length(y_r)
yrange <- range(y_r)
rlo <- floor(yrange[1] / orderOfMagnitude(yrange[1]) * 10) *
  orderOfMagnitude(yrange[1]) / 10
rhi <- ceiling(yrange[2] / orderOfMagnitude(yrange[2]) *10) *
  orderOfMagnitude(yrange[2]) / 10
...
# Map plot
plt_pH <- plot_fun(pH_na, Title = 'pH declining trends',
                   legend_title = 'stress')
plt_pH$plot
...
# Notes on treatment of facilities & shipping -----------------------------
# The impact of facilities and ship traffic was assessed in various ways before
# deciding upon the best approach. This is why there are three code sections,
# and why some of those code sections contain multiple methods.
# In the end, the selected approach was to use identical units
# (person days / year / area) for facilities and ship traffic, and to calculate
# this metric within every grid cell by adopting a smoothing method that
# estimates effective population size in all grid cells regardless of whether 
# they contain facilities or measured ship activity. This approach was used to
# reduce the impact of arbitrariness in the choice of grid cell size.
# Running all of the below code in the order that it is written will result in
# mapped outputs that use this method, however, other methods may be run through
# first.
# Facilities --------------------------------------------------------------
...
# not have data on population size. These can either be omitted from the
# analyses, or infilled using mean values.
...
for(i in 1:nranks){
  if(i < nranks){
    j <- r[i] <= stations$Peak_Population_adjusted &
      stations$Peak_Population_adjusted < r[i+1]} else{
      j <- r[i] <= stations$Peak_Population_adjusted &
        stations$Peak_Population_adjusted <= r[i+1]
    }
...
# Distance from coast -- these distances were crudely estimated from lo-res
# coastline data
...
for(i in 1:nranks){
  if(i < nranks){
    j <- r[i] >= stations$Distance_From_Coast &
      stations$Distance_From_Coast > r[i+1]} else{
        j <- r[i] >= stations$Distance_From_Coast &
          stations$Distance_From_Coast >= r[i+1]
      }
...
if(attr(dist2cells, 'units')$numerator == 'm'){
  dist2cells <- dist2cells * 1e-3} # convert m to km
# convert class to standard matrix
dist2cells <- matrix(as.numeric(dist2cells),
                     nrow = ngrid_cells, ncol = nfacilities)
# distance (km) of facility to nearest grid cell
minDist2cell <- apply(dist2cells, 2, min)
nearest_cell <- apply(dist2cells, 2, which.min)
# all facilities within a mapped grid cell
stationsInCells <- st_intersection(stations, grid_cells)
ind <- stations$order %in% stationsInCells$order
# Indicate the grid cell containing each facility - NA indicates inland
# facilities not contained in a grid cell
...
for(i in 1:length(cellsWithFacilities)){
  # label of a single cell containing facilities
  cell <- cellsWithFacilities[i]
  # facilities (rows of stations) in the cell
  facilities <- which(stations$cell_index == cell)
  stationsInCell[[which(grid_cells$cell_index == cell)]] <- facilities
}
# NULL values indicate grid cells not containing facilities
grid_cells$stations <- stationsInCell
...
# this distance, and the Earth radius, we can calculate the area of the (globe
# surface) circle surrounding a facility and intersecting the grid cell
# centroid. Within this area, A, the population density associated with the
# facility is Pd = P/A, where P is facility population. To find the associated
# 'population density', Pg, within the intersecting grid cell, we find the ratio
# of grid cell area, Ag, to circle area, A, then multiply: Pg = (Ag/A)(P/A).
# Something to bear in mind is that, if the circle intersects the grid cell
# centroid, only a fraction of the grid cell is enclosed within the circle, so
# dividing the grid cell area by 2 will produce a better estimate =>
# Pg = (1/2)(Ag/A)(P/A) people per km^2. Then, for each grid cell, these
# population densities are summed over all facilities. It may also be useful to
# include some limit on the radius used when calculating areas, say, 1000km.
# Also, circles that intersect large areas of land may require special attention.

circleAreaOnGlobe <- function(s, R = 6371, distanceLimit = NULL){
  # Returns surface area of circle defined by 'radius' s. The 'radius' is the
  # arc length between two points on the globe. R is average Earth radius (km);
  # s should have units of km; returned area has units of km^2.
...
distanceLimit <- NULL
# Set distance limit, beyond which facility populations do not have impact
# distanceLimit <- 1000

# Area of the circles centred on facilities and intersecting grid cell centroids
station2cellCircleArea <- circleAreaOnGlobe(
  dist2cells, distanceLimit = distanceLimit) # km^2
# Population density (people/km^2) within circle centered at facility and
# intersecting grid cells
popDensity_circle <- sweep(1 / station2cellCircleArea, 2,
                           stations$Peak_Population_adjusted, '*')
# Scaling factor -- ratio of grid cell area to circle area
cell2circleAreaRatio <- sweep(1 / station2cellCircleArea, 1,
                              grid_cells$area, '*')
# Circles intersect centroids of grid cells, so we need another scaling factor
# representing the approximate(?) fraction of grid cell contained in circle
modAreaRatio <- 0.393
...
# The above method is appropriate only for grid cells that are distant from
# facilities. The method breaks down for the grid cells containing facilities,
# but for these cells we can directly calculate population density. For each
# cell containing facilities, recalculate the population density
for(i in 1:length(cellsWithFacilities)){
  cell <- cellsWithFacilities[i]
...
# As with previous data, specify some ranking for disturbance in grid cells
# associated with population density.
y <- sort(grid_cells$popDensity) # person days / year / 10000 km^2
x <- 1:length(y)
# Temporarily exclude cells with population densities less than
# 1 person day / year / 10,000 km^2
y_ <- y[y>=1]
...
rhi <- floor(y_r[length(y_r)] / orderOfMagnitude(y_r[length(y_r)])) *
  orderOfMagnitude(y_r[length(y_r)])
rank_limits_stations <- 10 ^ seq(log10(rlo), log10(rhi), length = nranks + 1)
rank_limits_stations <- round(
  rank_limits_stations / {orderOfMagnitude(rank_limits_stations) / 10}) *
  {orderOfMagnitude(rank_limits_stations) / 10}
...
# Map plot
pltStations <- plot_fun_stations(
  data_stations = stations, data_grid_cells = grid_cells_reduced,
...
plt_stations_raw <- 
  ...
  geom_point(aes(x = index, y = popDensity, fill = rank, colour = rank),
  ...
  xlab(expression(grid ~ cell ~ index)) +
  ylab(expression(
    pop. ~ density ~ (PD ~ y ^ {'-1'} ~ (10^4 ~ km^2) ^ {'-1'}))) +
  scale_y_continuous(trans = 'log10', labels = function(x){
    f <- format(x, scientific = FALSE)
    while(any(grepl(' ', f))) f <- gsub(' ', '', f)
    f}) +
...
# area. Choosing person days allows more direct comparison with the facility
# data, however, comparing the person days will also involve some infilling.
...
# To match the grid cell population densities associated to facilities, convert
# the ship traffic unit from person days / year to
# person days / year / 10,000 km^2
ship_na <- ship_na_[ship_na_$variable == metric,]
...
plt_ship_raw <- 
  ...
  xlab(expression(grid ~ cell ~ index)) +
  ylab(
    expression(ship ~ traffic ~ (PD ~ y ^ {'-1'} ~ (10^4 ~ km^2) ^ {'-1'}))) +
  scale_y_continuous(trans = 'log10', labels = function(x){
    f <- format(x, scientific = FALSE)
    while(any(grepl(' ', f))) f <- gsub(' ', '', f)
    f}) +
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Now estimate ship-based person days for all grid cells using the area
# calculations from the Facilities section. This should be simpler than the
# facilities because ship data is centered within ocean grid cells, so much of
# the above code can be commented out.
grid_cells <- st_transform(grid_cells_all, crs = crs_world)
grid_cells <- st_make_valid(grid_cells)
grid_centroids <- st_centroid(grid_cells)
...
# Convert class to standard matrix
dist2cells <- matrix(
  as.numeric(dist2cells),
  nrow = ngrid_cells, ncol = nshipcells)

# List the ships contained within each grid cell -- simpler than facilities
# because all cells (south of 60 degrees) contain ships, even if the measurement
# is zero.
...
# Area of the circles centred on ship cells and intersecting all grid cell
# centroids
ship2cellCircleArea <- circleAreaOnGlobe(
  dist2cells, distanceLimit = distanceLimit) # km^2
# Population density (person days / year / km^2) within circle centered at
# facility and intersecting grid cells
...
# As circles intersect centroids of grid cells, we need another scaling factor
# representing the approximate(?) fraction of grid cell contained in circle
modAreaRatio <- 0.393
# Each ship cell's contribution to grid cell population density
# (person day / year / km^2)
popDensity_cell <- modAreaRatio * cell2circleAreaRatio * popDensity_circle

# The above method is appropriate only for grid cells that are distant from ship
# cells. The method breaks down for the grid cells containing ships, but for
# these cells we can directly calculate population density. For each cell
# containing ships, recalculate the population density.
for(i in 1:length(cellsWithShips)){
  cell <- cellsWithShips[i]
  ind <- grid_cells$cell_index == cell
  a <- grid_cells$area[ind]
...
y_ <- y[y > 0] # measurement zeros are lowest rank: not needed after smooth
# Cells with zero population density are not assigned a rank and will not appear
# on the map. This only applies when distanceLimit is set.
x_ <- 1:length(y_) / length(y_)
plot(x_, y_, log = 'y')
...
rank_limits_ship <- round(
  rank_limits_ship / {orderOfMagnitude(rank_limits_ship) / 10}) *
  {orderOfMagnitude(rank_limits_ship) / 10}
...
# Map plot
plt_ship <- plot_fun(grid_cells, Title = 'Ship traffic',
...
# Use the same units (person days / year / area) for facilities and ship traffic
# and also define the threshold values identically so that the rankings are
# comparable between these two data sets.

stations_c <- grid_cells_reduced_stations
ship_c <- grid_cells_reduced_ship

# merge values from both data sets (both have unit
# person days / year / 10,000 km2)
y <- sort(setNames(
  c(stations_c$popDensity, ship_c$value),
...
rhi <- floor(y_r[length(y_r)] / orderOfMagnitude(y_r[length(y_r)])) *
  orderOfMagnitude(y_r[length(y_r)])
rank_limits_stations_ships <- 10 ^ seq(log10(rlo), log10(rhi),
                                       length = nranks+1)
rank_limits_stations_ships <- round(
  rank_limits_stations_ships / 
    {orderOfMagnitude(rank_limits_stations_ships) / 10}) *
  {orderOfMagnitude(rank_limits_stations_ships) / 10}
...
# Map plots
pltStations_c <- plot_fun_stations(
  data_stations = stations, data_grid_cells = stations_c_reduced,
  rank_limits_PopSize = rank_limits_stationPopSize, Title = 'Facilities')
plt_ship_c <- plot_fun(ship_na, Title = 'Ship traffic',
                       legend_title = 'activity')
...
# Select which ship traffic metric, whether to estimate ship traffic for every
# cell, and whether to use make units identical to facilities
metric <- 'person time'
...
if(!sameRanks){
  stations <- grid_cells_reduced_stations
  }else stations <- stations_c_reduced
...
if(!sameRanks){
  plt_stations_raw <- plt_stations_raw
  }else plt_stations_raw <- plt_stations_c_raw
...
dat <- grid_cells_all[grid_cells_all$cell_index %in% all_polygons,]
...
plt_ship_station <- plot_fun(dat,
                             Title = combineTitle, legend_title = 'presence')
...
score_poly <- switch(
  ...
  mean = score_poly)
# create new data frame
dat <- grid_cells_all[grid_cells_all$cell_index %in% joint_polygons,]
dat$score <- score_poly
dat$rank <- score2rank(dat$score)
dat_interaction <- dat

plt_biota_activity <- plot_fun(dat_interaction, Title = combineTitle,
                               legend_title = 'risk')
...
plt_interaction <- ggdraw(arrangeGrob(
  ...
  plt_ship_station$legend,
  plt_biota_activity$plot + labs(title = 'Interaction') +
    theme(legend.position = 'none'),
  plt_biota_activity$legend,
  nrow = 1, widths = c(0.25, 1/12, 0.25, 1/12, 0.25, 1/12),
  top = 'Interaction with biota'))
plt_interaction
...
plt_stress_activity <- plot_fun(dat_multistressor, Title = combineTitle,
                                legend_title = 'risk')
...
plt_multistressor <- ggdraw(arrangeGrob(
  ...
  plt_sst_pH$legend,
  plt_ship_station$plot + labs(title = 'Activity') +
    theme(legend.position = 'none'),
  plt_ship_station$legend,
  plt_stress_activity$plot + labs(title = 'Interaction') +
    theme(legend.position = 'none'),
  plt_stress_activity$legend,
  nrow = 1, widths = c(0.25, 1/12, 0.25, 1/12, 0.25, 1/12),
  top = 'Contribution to stressors'))
plt_multistressor
...
plt_hotspot <- plot_fun(dat_hotspot, Title = combineTitle,
                        legend_title = 'risk')

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Combine and save these plots to generate an image for the main result
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# We need 6 plots: mean ranks for biota, stressors & activity on left column,
# and interaction ranks for biota-activity & stressors-activity, and overall
# interaction rank on the right column.
