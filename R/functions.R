...
createPolygons <- function(dat){
  ...
  # 'lonmax', 'latmin', and 'latmax' that define the area corresponding the
  # measurement variable(s). (I think this function may be too complicated --
  # perhaps we only need call to 'Polygon' function, and not the 'Polygons'
...
getMapData <- function(
  ...
  if(!is.null(mapSource)) dataDirectory <- paste(dataDirectory, mapSource,
                                                 sep = '/')
  if(!dir.exists(dataDirectory)) stop(
    paste0('Specified dataDirectory (', dataDirectory, ') does not exist!'))
  ...
  if(!anySubDirectories){
    ...
    coastlineFile <- paste(dataDirectory, coastDir, fileNames$coastline,
                           sep = '/')
    landFile      <- paste(dataDirectory, landDir,  fileNames$land, sep = '/')
    oceanFile     <- paste(dataDirectory, oceanDir, fileNames$ocean, sep = '/')
    iceshelfFile  <- paste(dataDirectory, iceDir,   fileNames$iceshelf,
                           sep = '/')
  }
  ...
  if(!mapIsLoaded){
    ...
    mapLimit_line <- lapply(1:nMaps, function(z){
      x <- matrix(
        c(Longitude = seq(lon_lim[1], lon_lim[2],
                          length.out = 10 * diff(lon_lim)),
    ...
    # z=1
    # ggplot() + geom_sf(data = subset(dat[[z]], feature == 'land'),
    # fill = 'darkgreen') + geom_sf(data = subset(dat[[z]], feature == 'ice'),
    # fill = 'lightblue') + geom_sf(data = subset(dat[[z]],
    # feature == 'coastline'), colour = 'black')
    ...
    # Create an ocean polygon -- it's tricky to eliminate narrow edges between
    # adjacent polygons. The land and ice polygons are not perfectly aligned so
    # ocean can creep into the gaps. Solve this by expanding the coastline
    # points into tiny polygons then merging all polygons to mask all dry
    # regions.
    ...
    x <- lapply(1:nMaps, function(z){
      outer(xy[[z]][,'X'], c(-1,1) * diff(bb[[z]][c('xmin','xmax')]) * 1e-3,
            '+')})
    y <- lapply(1:nMaps, function(z){
      outer(xy[[z]][,'Y'], c(-1,1) * diff(bb[[z]][c('ymin','ymax')]) * 1e-3,
            '+')})
      ...
      lapply(1:nxy[[w]], function(z){
        Polygons(list(
          Polygon(matrix(c(xw[z,1], yw[z,1],
                           ...
                           xw[z,2], yw[z,2],
                           xw[z,1], yw[z,2],
                           xw[z,1], yw[z,1]), ncol = 2, byrow = TRUE))),
          ids[[w]][z])
      })
    })
    ...
    ocean <- lapply(1:nMaps, function(z){
      w <- st_difference(mapLimit_poly[[z]],
                         st_make_valid(st_union(rbind(dat[[z]], xy[[z]]))))
      w$feature <- 'ocean'
      w[c('feature','geometry')]})
    dat <- lapply(1:nMaps, function(z) rbind(dat[[z]], ocean[[z]]))

    # z = 2
    # ggplot() + geom_sf(data = subset(dat[[z]], feature == 'ocean'),
    # fill = 'blue') + geom_sf(data = subset(dat[[z]], feature == 'land'),
    # fill = 'darkgreen') + geom_sf(data = subset(dat[[z]], feature == 'ice'),
    # fill = 'lightblue') + geom_sf(data = subset(dat[[z]],
    # feature == 'coastline'), colour = 'black') + 
    # geom_sf(data = subset(dat[[z]], feature == 'boundary'), colour = 'black',
    # linewidth = 1)
  ...
  output$map_data <- setNames(
    dat, paste('lat',
               sapply(1:nMaps, function(z) paste(lat_lim[[z]], collapse = '_')),
               sep = '_'))
  if(createGrid){
...
get_data <- function(
    ...
    SST_overallTrend = TRUE, pH_overallTrend = TRUE, sigDigits = 2,
    roundShipTime = FALSE, shipOrPersonTime = NULL, indexGridCells = TRUE,
    loadTooltipFromFile = TRUE, verbose = FALSE, theseData = NULL){
  # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if(indexGridCells){
    ...
    grid_cells_all <- SpatialPolygons(
      grid_cells_all,
      proj4string = CRS(st_as_text(crs_world))) # create spatial object
    ...
    if(all(attr(grid_cell_areas, 'units')$numerator == 'm')){
      grid_cell_areas <- grid_cell_areas * 1e-6}  # convert m^2 to km^2
  ...
  switch(shipSummaryDataOrRaw,
         summary = {
           ...
           f <- 'McCarthy_2022/so_ecoregion_combined.csv' # data table with all
                                                          # ship types
           ...
           measure_vars <- c("n_voyages", "n_ships", "total_time",
                             "median_time", "mean_time", "n_time" )
           ship_poly <- do.call(
             'rbind',
             lapply(1:n_ship_classes, function(z){
               ...
               sp <- ship_poly_
               sp$ship_class <- sapply(sp$ECO_CODE_X, function(z){
                 d$ship_class[d$ecoregion_number == z]})
               sp$n_voyages <- sapply(sp$ECO_CODE_X, function(z){
                 d$n_voyages[d$ecoregion_number == z]})
               sp$n_ships <- sapply(sp$ECO_CODE_X, function(z){
                 d$n_ships[d$ecoregion_number == z]})
               sp$total_time <- sapply(sp$ECO_CODE_X, function(z){
                 d$total_time[d$ecoregion_number == z]})
               sp$median_time <- sapply(sp$ECO_CODE_X, function(z){
                 d$median_time[d$ecoregion_number == z]})
               sp$mean_time <- sapply(sp$ECO_CODE_X, function(z){
                 d$mean_time[d$ecoregion_number == z]})
               sp$n_time <- sapply(sp$ECO_CODE_X, function(z){
                 d$n_time[d$ecoregion_number == z]})
               ...
               if(is.list(sp$ship_class)){
                 ...
                 for(i in c('ship_class', measure_vars)){
                   sp[[i]] <- unlist(sp[[i]])}
               }
               sp
             })
         ...
         raw = {
           ...
           ship <- aggregate(
             cbind(ship_time, person_time) ~ lonmin + lonmax + latmin + latmax + 
               year + activity_type, data = ship, FUN = sum)
           
           # Sum ship time over vessel activity
           ship_ <- aggregate(
             cbind(ship_time, person_time) ~ lonmin + lonmax + latmin + latmax + 
               year, data = ship, FUN = sum)
           ...
           ship_ <- aggregate(
             cbind(ship_time, person_time) ~ lonmin + lonmax + latmin + latmax +
               activity_type, data = ship, FUN = mean)
           ship_$year <- 'all'
           ship_ <- ship_[names(ship)]
           ship <- rbind(ship, ship_)
           
           # Use the annual average only, discard data pertaining to specific
           # years
           ship <- subset(ship, year == 'all')
           
           if(roundShipTime){
             # Round ship_time to nearest day -- this removes grid cells with
             # negligible ship presence, creating a more useful colour scale
             # with greater focus on areas of high traffic.
             # I only want to crop the small measures, so only round numbers
             # less than one.
           ...
           if(is.null(shipOrPersonTime)){
             ...
             if(!shipOrPersonTime %in% c('ship', 'person')){
               error("'shipOrPersonTime' must be either 'ship' or 'person'")}
             ship <- switch(shipOrPersonTime,
                            ship = ship[,names(ship) != 'person_time'],
                            person = ship[,names(ship) != 'ship_time'])
             ship <- melt(
               ship, measure.vars = paste0(shipOrPersonTime, '_time'))
           }
          ...
          lat_max_ship <- max(ship$latmax)
           ...
           if(indexGridCells){
             ...
             ship_poly$cell_index <- grid_cells_all$cell_index[
               apply(distBetweenCells, 2, which.min)]
           }
           ship_poly <- st_transform(
             ship_poly, crs = crs_use) # convert coordinates
  ...
  DATA_names_spaces[c(2:9, 11:12)] <- c(
    'Sample type','Sample gear','Litter ID method','Litter category',
    'Litter scale','Plastic form','Plastic size','Sample at station',
    'Site category','Sample ID')
  DATA_name_swap <- as.data.frame(
    cbind(original = DATA_names_orig, spaces = DATA_names_spaces))
  ...
  seawater <- allSampleTypes[grepl('sea', allSampleTypes) |
                               grepl('marine', allSampleTypes)]
  freshwater <- allSampleTypes[grepl('fresh', allSampleTypes) |
                                 grepl('waste', allSampleTypes)]
  ...
  DATA$SampleType_grouped[
    DATA$SampleType %in% freshwater] <- 'freshwater/wastewater'
  ...
  DATA$SampleType_grouped <- factor(
    DATA$SampleType_grouped,
    levels = c('seawater','freshwater/wastewater','sediment','ice','air'))
  ...
  sourceYear <- suppressWarnings(
    as.integer(sapply(
      1:nsources, FUN = function(z) substr(sources[z], ncs[z]-4, ncs[z]-1)))
  )
  DATA$Source <- factor(DATA$Source, levels = sources[order(sourceYear)])
  
  # Column for interactive (mouse scrolling) labeling of position. Samples at a
  # fixed station have single lat-lon coordinate; longer tows have start/end
  # coords.
  # Fixed stations...
  ...
  DATA$Coordinates[as.logical(DATA$SampleAtStation)] <- coord_lab[
    as.logical(DATA$SampleAtStation)]
  # Longer tows...
  ...
  DATA$Variable <- factor(
    DATA$Variable, 
    levels = c('concentration', 'density', 'mass concentration', 'mass density',
               'flux', 'presence/absence'))
  
  plastic_units <- unique(DATA[,c('SampleType', 'Variable','Unit')])
  plastic_units$SampleType <- as.character(plastic_units$SampleType)
  plastic_units$SampleType[
    grepl('water', plastic_units$SampleType) | 
      grepl('marine', plastic_units$SampleType)] <- 'water'
  plastic_units <- unique(plastic_units)
  plastic_units$SampleType <- factor(
    plastic_units$SampleType, levels = unique(plastic_units$SampleType))
  ...
  # PlasticForm is important -- R shiny filtering should be among a few groups
  # (fragment, fibre, film, other/unspecified), but the data display should
  ...
  film <- c('film')
  other <- allPlasticForms[!allPlasticForms %in% 
                             c(fragments, fibres, film, 'all')]
  ...
  DATA$PlasticForm <- factor(DATA$PlasticForm,
                             levels = c(fragments, fibres, film, other, 'all'))
  DATA$PlasticForm_grouped <- factor(
    DATA$PlasticForm_grouped,
    levels = c('fragment','fibre','film','other/unspecified','all'))
  
  # Plastic scale
  DATA$LitterScale <- as.character(DATA$LitterScale)
  DATA$LitterScale[grepl(
    'micro and meso', DATA$LitterScale)] <- 'micro and meso'
  DATA$LitterScale[DATA$LitterScale %in% c('see size data','')] <- 'unspecified'
  DATA$LitterScale <- factor(
    DATA$LitterScale,
    levels = c('micro', 'meso', 'micro and meso', 'macro', 'unspecified'))

  # Plastic size
  DATA$PlasticSize <- as.character(DATA$PlasticSize)
  DATA$PlasticSize[DATA$PlasticSize == ''] <- 'unspecified'
  DATA$PlasticSize <- factor(DATA$PlasticSize,
                             levels = unique(DATA$PlasticSize))

  # Litter category
  DATA$LitterCategory <- as.character(DATA$LitterCategory)
  if(!allLitterTypes){ # only include plastic litter
  ...
  DATA$Statistic <- factor(DATA$Statistic,
                           levels = c('raw','mean','s.d.','min','max','none'))
  
  # Set non-numeric Values to -Inf to distinguish them from NAs.
  # This is for presence/absence data.
  DATA$Value[is.na(DATA$Value) & DATA$Variable == 'presence/absence'] <- -Inf

  # Order the data
  DATA <- DATA[order(DATA$Source, DATA$SampleType, DATA$SampleID,
                     DATA$LitterScale, DATA$PlasticSize, DATA$PlasticForm,
                     DATA$Variable, DATA$Statistic, DATA$Replicate),]
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # Interactive tooltip showing plastic sample info as table is a code
  # bottle-neck. To avoid slow map rendering, save data with tooltip variable
  # to file.
  ...
  if(!loadTooltipFromFile){
    ...
    yvars <- c(
      'LitterCategory', 'LitterScale', 'PlasticSize', 'PlasticForm',
      'Variable', 'Statistic', 'Replicate', 'Unit') # variables to change to
                                                    # wide format -- displayed
                                                    # over multiple rows
    xvars <- dn[
      !dn %in% c(
        yvars, 'Value', dummyGroupingVariables)] # everything else, apart from
                                                 # Value and dummy grouping
                                                 # variables, stays in long
                                                 # format
    # Choose other variables to display in a single row.
    keepVars <- c('Source', 'SampleGear', 'Depth', 'Sample date', 'Coordinates',
                  'Coordinates (start)', 'Coordinates (end)', 'SampleAtStation',
                  'Observation')
    # Some variables require multiple rows depending on the data source/sample
        ...
        any(sapply(1:length(atStation), function(w){
          ...
          any(sapply(1:length(sampleTypes), function(x){
            ...
            any(sapply(1:length(allSamples), function(y){
              length(unique(d2[[keepVars[u]]][
                d2$SampleID == allSamples[y] & 
                  d2$SampleType == sampleTypes[x]])) > 1
            }))
          }))
        }))
      }))
    ...
    DATA_wide$tooltip <- sapply(1:length(temp_dat), function(z) fun_flextable(
      x = temp_dat[[z]], longVars = yvars, singleRowVars = keepVars,
      sampleType = sType[z], DATA_name_swap))
    ...
    # Remove NAs
    mv <- unique(DATA$data_id[is.na(DATA$Value)]) # samples with missing values
    j <- (DATA_long$data_id %in% mv) # index melted data with missing samples
    ds <- DATA[DATA$data_id %in% mv, c('data_id', yvars)] # discard rows not in
                                                          # original data
    ...
    if(saveTooltipData){
      fn <- paste0(paste(sub('.csv', '', f), 'tooltip', 'wide', sep = '_'),
                   '.Rds')
      saveRDS(DATA_wide, fn)
      fn <- paste0(paste(sub('.csv', '', f), 'tooltip', 'long', sep = '_'),
                   '.Rds')
  ...
  # Mapping coordinates
  DATA_sf <- st_as_sf(DATA_wide, coords = c('Longitude', 'Latitude'),
                      crs = crs_world)
  ...
  STATIONS$Type <- reorder(STATIONS$Type, rep(1, nrow(STATIONS)), sum,
                           decreasing = TRUE)
  
  # Define tooltip variable for interactive info...
  
  # Attribute values cannot contain single quotes "'", so remove them.
  ...
  # Mapping coordinates
  STATIONS_sf <- st_as_sf(STATIONS, coords = c('Longitude_DD', 'Latitude_DD'),
                          crs = crs_world)
  ...
  station_coords[c('Longitude_rad','Latitude_rad')] <- pi / 180 * station_coords
  
  STATIONS_sf$Distance_From_Coast <- sapply(1:nrow(station_coords), function(z)
    min(greatCircDist(coastPoints[,1], station_coords$Longitude_rad[z],
                      coastPoints[,2], station_coords$Latitude_rad[z],
                      r = 6371))
  )
  
  STATIONS_sf$tooltip <- paste0(
    STATIONS_sf$Official_Name, ' ', STATIONS_sf$Type, '\n ',
  ...
  KRILL_allMonths <- aggregate(
    value ~ lonmin + lonmax + latmin + latmax, KRILL, 'mean')
  ...
  krill_poly <- SpatialPolygons(
    krill_poly, proj4string = CRS(st_as_text(crs_world))) # make spatial object
  ...
  if(indexGridCells){
    ...
    krill_poly$cell_index <- grid_cells_all$cell_index[
      apply(distBetweenCells, 2, which.min)]
  }
  ...
  # create a discrete colour scheme based on krill (log scale) abundance
  ncols <- 10
  clims <- 2 ^ {1:ncols-1}
  clims[1] <- 0
  ...
  # Mean abundance across months for each grid cell
  CHL_allMonths <- aggregate(
    value ~ lonmin + lonmax + latmin + latmax, CHL, 'mean')
  ...
  chl_poly <- SpatialPolygons(
    chl_poly, proj4string = CRS(st_as_text(crs_world))) # create spatial object
  chl_poly <- st_as_sf(chl_poly)
  if(indexGridCells){
    # Match polygons to those listed in grid_cells_all
    chl_centroids <- st_centroid(chl_poly)
    distBetweenCells <- st_distance(grid_centroids, chl_centroids)
    chl_poly$cell_index <- grid_cells_all$cell_index[
      apply(distBetweenCells, 2, which.min)]
  }
  ...
  if(SST_overallTrend){
    filename <- paste0('sst_time_series_trend_res_', res, '.csv')
  }else{
  ...
  if(SST_overallTrend){
    ...
    SST <- cbind(SST[,coords], metric = 'trend', value = SST$linearTrend,
                 pval = SST$pValue)
    ...
    siglabels <- c(paste('p <=', significanceContours),
                   paste('p >', tail(significanceContours, 1)))
    ...
    if(sstType == 'anomaly'){
      ...
      SST_allMonths <- aggregate(
        value ~ lonmin + lonmax + latmin + latmax, SST, 'mean')
    ...
    if(sstType == 'trend'){
      ...
      SST <- melt(SST, id.vars = coords, variable.name = 'month',
                  value.name = 'value')
      pvals <- melt(pvals, id.vars = coords, variable.name = 'month',
                    value.name = 'value')
      ...
      # p-values, define p-level as a category variable
      nsiglevels <- length(significanceContours) + 1
      SST$plevel <- apply(outer(SST$pval, significanceContours, '>'), 
                          1, sum) + 1
      siglabels <- c(paste('p <=', significanceContours),
                     paste('p >', tail(significanceContours, 1)))
      SST$plevel <- factor(SST$plevel,
                           levels = 1:nsiglevels, labels = siglabels)
  ...
  sst_poly <- SpatialPolygons(sst_poly,
                              proj4string = CRS(st_as_text(crs_world)))
  ...
  if(indexGridCells){
    ...
    sst_poly$cell_index <- grid_cells_all$cell_index[
      apply(distBetweenCells, 2, which.min)]
  }
  ...
  if(pH_overallTrend){
    filename <- paste0('pH_time_series_trend_res_', res, '.csv')
  }else{
  ...
  if(pH_overallTrend){
    ...
    coords <- c('lonmin','lonmax','latmin','latmax')
    pH <- cbind(pH[,coords], metric = 'trend', value = pH$linearTrend,
                pval = pH$pValue)
    ...
    if(pHType == 'anomaly'){
      ...
      pH_allMonths <- aggregate(
        value ~ lonmin + lonmax + latmin + latmax, pH, 'mean')
    ...
    if(pHType == 'trend'){
      ...
      pH <- melt(pH, id.vars = coords, variable.name = 'month',
                 value.name = 'value')
      pvals <- melt(pvals, id.vars = coords, variable.name = 'month',
                    value.name = 'value')
      ...
      # A small number of grid cells have p-value = NA due to few (only 2?)
      # measurements -- set these p-values to 1
  ...
  pH_poly <- SpatialPolygons(pH_poly, proj4string = CRS(st_as_text(crs_world)))
  ...
  if(indexGridCells){
    ...
    pH_poly$cell_index <- grid_cells_all$cell_index[
      apply(distBetweenCells, 2, which.min)]
  }
  ...
  lat_lim_cells <- c(-90, max(lat_max_sst))
  ...
  if(!indexGridCells){
    ...
    relabelFun <- function(dat){
      sapply(1:nrow(dat),
             function(z){
               relabelCells$new[relabelCells$old == dat$cell_index[z]]})}
    if(is.null(theseData)){
    ...
    # convert coordinates
    grid_cells_all <- st_transform(grid_cells_all, crs = crs_use)
...
fun_flextable <- function(
  ...
  # Function defining tables to interactively display when mouse hovers over
  # mapped (plastic) data points.
  # Inputs: x = data, pu = plastic_units, singleRowVars = variables only taking
  # one row of table.
  # (Some nice examples, worth a look, presented here 
  ...
  if(anyMeasures){
    ...
    if(all(c('min','max') %in% x$Statistic)){ # convert min/max into range
      ...
      n <- nrow(x_)
      v <- sapply(seq(1, n, 2), function(z){
        paste0('[', x_$value[z], ', ', x_$value[z+1], ']')})
      x$Value[x$Statistic == 'min'] <- v
      x$Statistic[x$Statistic == 'min'] <- 'range'
      x <- x[x$Statistic != 'max',]
    }
    if(all(c('mean','s.d.') %in% x$Statistic)){
      # convert mean/s.d. into mean +- s.d.
    ...
    x$Measure <- x$Variable
    n <- nrow(x)
    if(x$SampleAtStation[1]){
      singleRowVars <- singleRowVars[
        !singleRowVars %in% c('Coordinates (start)', 'Coordinates (end)',
                              'SampleAtStation')]
    }else{
      singleRowVars <- singleRowVars[
        !singleRowVars %in% c('Coordinates', 'SampleAtStation')]}
    if(any(x$Unit == 'none')){
    ...
    for(j in 1:length(singleRowVars)){
      x[[singleRowVars[j]]] <- as.character(x[[singleRowVars[j]]])}
    
    if(n > 1) x[singleRowVars][2:n,] <- ''
    # Include spaces in column names
    newnames <- sapply(1:ncol(x), function(z){
      n <- names(x)[z]
      if(n %in% DATA_name_swap$original){
        m <- DATA_name_swap$spaces[n == DATA_name_swap$original]
      }else m <- n
      c(n,m)
    })
    names(x) <- newnames[2,]
...
set_plot_params <- function(base_map, plastics, stations){
  ...
  pltSymbols <- data.frame(
    Class = rep('ResearchStation', nstationTypes), Type = stationTypes,
    symbol = pltShapes[1:nstationTypes])
  ...
  pltSymbols <- rbind(pltSymbols,
                      data.frame(
                        Class = rep('PlasticSample', nsampleTypes),
                        Type = sampleTypes, symbol = pltShapes[1:nsampleTypes]))
  ...
  blues <- c('#A6E5FF', '#002BFF')
  greens <- c('#B0FBD5', '#004D26')
  reds <- c('#FFECEF', '#990000')
...
make_plot <- function(
    ...
    stationsOnly = FALSE, plasticOnly = FALSE, ptSize = 4, legPtSize = 4,
  ...
  if(background.na.remove)
    dat_background <- dat_background[!is.na(dat_background$value),]
  ...
  e2sci <- function(x, sig.dig = 2, lim = 4){
    ...
    if(any(small)){
      ...
      if(sig.dig == 1)
        y_ <- substr(y_, 1, 1)
      if(sig.dig > 1)
        y_ <- paste(substr(y_, 1, 1), substr(y_, 2, sig.dig), sep = '.')
         ...
         krill = {
           ...
           if(!discreteColourScheme){
             ...
             dat_background$value[dat_background$value == 0] <-
               0.5 * min(dat_background$value[dat_background$value > 0],
                         na.rm = TRUE)
             ...
             if(autoScaleColours){
               ...
               # Create break points
               dat_background$breaks <- cut(
                 dat_background$value_log10, nColours)
               ...
               brks[nbrks] <- ceiling(
                 brks[nbrks] / mag_0.5[nbrks]) * mag_0.5[nbrks]
               brks_ <- e2sci(brks)
               legLabs <- sapply(
                 1:nColours, function(z){
               ...
               levelsInData <- levels(dat_background$breaks) %in%
                 dat_background$breaks
               
               dat_background$breaks <- factor(
                 dat_background$breaks,
               ...
               levelsInData <- levels(
                 dat_background$breaks) %in% dat_background$breaks
         ...
         pH = {
           ...
           if(!discreteColourScheme){
               ...
               if(weblink_exists){
                 ...
                 plt_background <- plt_background + 
                   scale_fill_gradientn(
                     colours = rev(Cols[{ncol_pH_+1}:ncol_pH]),
                     values = cval, name = legLabel,
                     na.value = na.colour)
               }
             }}}
             ...
             if(any_positive & any_negative){
               ...
               dat_background$breaks <- factor(
                 dat_background$breaks, levels = levels(dat_background$breaks))
               ...
               if(weblink_exists){
                 ...
                 plt_background <- plt_background +
                   scale_fill_manual(
                     values = Cols[levelsInData], name = legLabel,
                     na.value = na.colour,
                     labels = c(levels(dat_background$breaks)[levelsInData],
                                'no data'))
               }
               ...
               if(weblink_exists){
                 ...
                 plt_background <- plt_background +
                   scale_fill_manual(
                     values = rev(Cols_pos), name = legLabel,
                     na.value = na.colour,
                     labels = c(levels(dat_background$breaks), 'no data'))
               }
             }}}
               ...
               if(!discreteColourScheme){
                 ...
                 if(all(manual_legend_breaks == FALSE)){
                   if(!anyZeros){
                     ...
                     brks <- seq(min(dat_background$value_log10[i]), 
                                 max(dat_background$value_log10[i]),
                                 length.out = nColours)
                     mag <- 10 ^ floor(brks)
                     mag_ <- 0.5 * mag
                     brks <- 10 ^ brks
  ...
  if(plotSignificanceContours){
    if(background %in% c('sst', 'pH')){
      ...
      centroids <- as.data.frame(st_coordinates(
        st_centroid(dat_$geometry))) # get coordinates of polygon centroids
      ...
      # Transform to stereogrpahic coordinates
  ...
  plt_map <- plt_background + guides(fill = 'none') + guides(linetype = 'none')
  
  # Ecoregions - this option is deprecated
  ...
  if(backgroundOnly){
    if(showCoordGrid) plt_map <- overlay_coordinate_grid(
      plt_map, singleLatAxis = singleLatAxis, latlim = latlim,
      textSize = axisTextSize)
    if(include_overlay_labels) plt_map <- overlay_labels_plot(
      plt_map, overlay_labels)
    return(
  ...
  if(anyPlastic){
    ...
    plt_plastic_samples <-
      ...
      theme(legend.key = element_blank(),
            legend.justification = 'left')
    leg_plastic <- get_plot_component(plt_plastic_samples,
                                      'guide-box', return_all = TRUE)[[1]]
    # get legend dimensions
  ...
  leg_height <- {3 * 2} + max(c(leg_plastic_height,
                                leg_stations_height + leg_background_height))
  ...
  # Use patchwork to directly control layout
  ...
  if(lpad > 0){
    ...
    lph <- c(rep('#', floor(0.5 * abs(lpad))), lph,
             rep('#', ceiling(0.5 * abs(lpad))))
  }
  lp <- rep('#',length(lph))
...
plot_fun <- function(data, n = nranks, v_option = 'viridis', v_direction = -1,
  ...
  plt <-
    ...
    scale_fill_viridis_d(option = v_option, direction = v_direction,
                         ...
                         na.value = na.colour,
                         labels = c(levels(data$rank), 'no data')) +
    theme(
      axis.text = element_blank(),
      axis.ticks = element_blank(),
      panel.background = element_rect(fill = 'white', colour = 'white'),
    ...
    geom_sf(data = nc,
            aes(fill = surface)) +
    scale_fill_manual(values = c(land.colour,ice.colour,ice.colour,land.colour),
                      guide = 'none')
...
plot_fun_stations <- function(
    ...
    rank_limits_PopSize = NULL, Title = NULL, show_legend = TRUE,
    showAxis = TRUE, latlim = NULL, axisTextSize = 4){
  ...
  if(is.null(rank_limits_PopSize)){
    ...
    data_stations$rank_pop_labels <- factor(
      data_stations$rank_pop, levels = 1:n, labels = rl)
  }
  
  seasons <- tolower(levels(data_stations$Seasonality))
  data_stations$Seasonality <- factor(
    data_stations$Seasonality, labels = seasons)
  
  data_grid_cells$rank <- factor(
    data_grid_cells$rank, levels = n:1, labels = n:1)
  ...
  plt <-
    ...
    geom_sf(data = data_grid_cells,
            aes(fill = rank)) +
    scale_fill_viridis_d(option = v_option, direction = v_direction,
                         name = 'activity', drop = FALSE, na.translate = FALSE,
    ...
    scale_fill_viridis_d(
      option = v_option, direction = v_direction, guide = 'none') +
    scale_fill_viridis_d(
      aesthetics = 'fill_new_new', option = v_option, direction = v_direction,
      name = 'activity', drop = FALSE, na.translate = FALSE,
      guide = guide_legend(override.aes = list(size = NA, shape = NA))) +
    scale_size_manual(
      values = 1:n, guide = guide_legend(
        title = 'population', order = 1,
        override.aes = list(fill = NA, linetype = 0))) +
    scale_shape_manual(
      values = c(21, 22), guide = guide_legend(
        title = 'seasonality', order = 2,
        override.aes = list(size = n, fill = NA, linetype = 0)))
  
  if(showAxis){
    assign('BBox', st_bbox(nc), envir = .GlobalEnv)
    plt <- overlay_coordinate_grid(
      plt, latlim = latlim, textSize = axisTextSize)
  }
